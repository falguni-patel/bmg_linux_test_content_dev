...
os.makedirs(os.path.dirname(SUPERPOSITION_LOG_FILEPATH), exist_ok=True)


# Create venv if not exists
if not os.path.exists(VENV_PATH):
    print("Creating virtual environment...")
    subprocess.run(f"python3 -m venv {VENV_PATH}", shell=True, executable="/bin/bash")

#  Activate venv and Install Required Packages
print("Activating virtual environment and installing dependencies...")
for pkg in REQUIREMENTS:
    subprocess.run(f"source {VENV_ACTIVATE} && pip install {pkg}", shell=True, executable="/bin/bash")

# Run the Unigine SUPERPOSITION script
print("Running Unigine SUPERPOSITION benchmark...")

SUPERPOSITION_command = f"export DISPLAY=:0 && cd {SUPERPOSITION_DIR} && source {VENV_ACTIVATE} && python3 {SUPERPOSITION_EXECUTION_SCRIPT} {execution_time}"
SUPERPOSITION_process = subprocess.Popen(SUPERPOSITION_command, shell=True, executable="/bin/bash")
...
while time.time() - start_time < execution_time:
    ...
    if elapsed_time % GPUTOP_INTERVAL < 1:  # Every 15 minutes
        ...
        gputop_log_filepath = os.path.join(HEAVEN_LOG_FILEPATH, gputop_log_filename)
