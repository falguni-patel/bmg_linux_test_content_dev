...
os.makedirs(os.path.dirname(VALLEY_LOG_FILEPATH), exist_ok=True)
...
# Run the Unigine VALLEY script
print("Running Unigine VALLEY benchmark...")

VALLEY_command = f"export DISPLAY=:0 && cd {VALLEY_DIR} && source {VENV_ACTIVATE} && python3 {VALLEY_EXECUTION_SCRIPT} {execution_time}"
VALLEY_process = subprocess.Popen(VALLEY_command, shell=True, executable="/bin/bash")
