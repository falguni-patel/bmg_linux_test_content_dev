...
os.makedirs(os.path.dirname(VLC_LOG_FILEPATH), exist_ok=True)
...
print("Running VLC benchmark...")

VLC_command = f"export DISPLAY=:0 && cd {VLC_DIR} && source {MEDIA_VENV_ACTIVATE} && python3 {VLC_EXECUTION_SCRIPT} {execution_time}"
VLC_process = subprocess.Popen(VLC_command, shell=True, executable="/bin/bash")
...
while time.time() - start_time < execution_time:
    ...
    if elapsed_time % GPUTOP_INTERVAL < 1:  # Every 15 minutes
        ...
        gputop_log_filepath = os.path.join(VLC_LOG_FILEPATH, gputop_log_filename)
